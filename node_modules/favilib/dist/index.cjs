...
async function createScreenshot(sourceset, name, options) {
  const { width, height } = sourceset[0].metadata;
  const [finalWidth, finalHeight] = options.sizes ? options.sizes.split("x").map(Number) : [width, height];
  const contents = await sharp__default(sourceset[0].data).resize(finalWidth, finalHeight, {
    fit: "cover",
    position: "center"
...
class AndroidPlatform extends Platform {
  ...
  async createHtml() {
    const manifestLink = this.options.loadManifestWithCredentials ? `<link rel="manifest" href="${this.cacheBusting(
      this.relative(this.manifestFileName())
    )}" crossorigin="use-credentials">` : `<link rel="manifest" href="${this.cacheBusting(
      this.relative(this.manifestFileName())
    )}">`;
    const themeMetaTags = Array.isArray(this.options.themes) && this.options.themes.length > 1 ? [
    ...
    const appNameMetaTag = this.options.name ? `<meta name="application-name" content="${escapeHtml__default(
      this.options.name
  ...
  async screenshots() {
    const screenshots = await Promise.all(
      this.options.screenshots.map(async (screenshot, index) => {
        ...
        const Format = type?.split("/")[1] ?? "webp";
  ...
  async manifest() {
    const { options } = this;
    const basePath = options.manifestRelativePaths ? null : options.output.assetsPrefix;
    const properties = {
      name: options.name,
    ...
    if (options.manifest.prefer_related_applications) {
      properties.prefer_related_applications = options.manifest.prefer_related_applications;
    }
    if (Array.isArray(options.manifest.related_applications) && options.manifest.related_applications.length > 0) {
  ...
  async manifestScreenshots(basePath) {
    const screenshots = await Promise.all(
      this.options.screenshots.map(async (screenshot, index) => {
        const {
          src,
        ...
        try {
          ...
          const calculatedFormFactor = form_factor ?? (calculatedWidth < 720 ? "narrow" : "wide");
          const prefix = platform ? `${platform}_` : "";
...
class AppleIconPlatform extends Platform {
  ...
  async createHtml() {
    const icons = this.iconOptions.map((options) => {
    ...
    return [
      ...
      name ? `<meta name="apple-mobile-web-app-title" content="${escapeHtml__default(
        name
...
class WindowsPlatform extends Platform {
  ...
  browserConfig() {
    const basePath = this.options.manifestRelativePaths ? null : this.options.output.assetsPrefix;
    const tile = {};
...
async function favicons(source, options = { name: null, short_name: null }) {
  options = {
    ...defaultOptions,
    ...options,
